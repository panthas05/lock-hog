...
class SlowReleaseThreadLockHogger(ThreadLockHogger):
    """
    The same as ThreadLockHogger, but waits for a second before releaseing the lock. We
...
class SlowReleaseProcessLockHogger(ProcessLockHogger):
    """
    The same as ProcessLockHogger, but waits for a second before releasing the lock. We
    do this only so that we can test the timeout handling.
    """
